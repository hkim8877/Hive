...
static void exec_and_error(t_list *pipex, char *cmd_path, char **cmd, char **envp)
{
...
static void	child_process_1(t_list *pipex, char **argv, char **envp, int *fd)
{
	...
	exec_and_error(pipex, cmd_path, pipex->cmd1, envp);
}
...
static void	child_process_2(t_list *pipex, char **argv, char **envp, int *fd)
{
	...
	exec_and_error(pipex, cmd_path, pipex->cmd2, envp);
}
// static void	child_process_1(t_list *pipex, char **argv, char **envp, int *fd)
// {
// 	char	*cmd_path;
...
// 			cleanup("No such directory or directory", pipex, 1);
// 		else
// 			cleanup("command not found", pipex, 127);
// 	}
...
// 		cmd_path = ft_strdup(pipex->cmd2[0]);
// 	else
// 		cmd_path = get_cmd_path(pipex->path, pipex->cmd2);
// 	if (!cmd_path)
// 	{
